...
export function useSubscription() {
    ...
    // Get current subscription and plan details from Inertia shared data
    const subscription = computed(() => page.props.subscription?.active || null);
    const plan = computed(() => subscription.value?.plan || null);
    const planName = computed(() => plan.value?.slug?.toLowerCase() || subscription.value?.plan_type?.toLowerCase() || 'free');
